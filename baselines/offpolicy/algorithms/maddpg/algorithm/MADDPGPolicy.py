...
class MADDPGPolicy(MLPPolicy):
    ...
    def get_actions(self, obs, available_actions=None, t_env=None, explore=False, use_target=False, use_gumbel=False):
        ...
        if self.discrete:
            ...
            # print("self.target_noise:", self.target_noise)
            # print("actor_out:", actor_out)
            if explore:
                # print("actor_out:", gaussian_noise(actor_out.shape, self.args.act_noise_std))
    ...
    def get_random_actions(self, obs, global_config, available_actions=None):
        """See parent class."""
        batch_size = obs.shape[0]
        ...
        if self.discrete:
            ...
            random_actions = np.random.uniform(-1, 1, size=(batch_size, self.act_dim))
            # random_actions[:, 0] = random_actions[:, 0] * global_config.base_station_set_config.base_station_num
            # random_actions[:, 0] = np.floor(random_actions[:, 0])
