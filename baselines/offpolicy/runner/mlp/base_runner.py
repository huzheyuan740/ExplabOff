...
class MlpRunner(object):
    ...
    def __init__(self, config):
        ...
        self.num_envs = 1  # self.env.num_envs
        self.num_eval_envs = 1  # self.eval_env.num_envs

        # dir
        self.model_dir = self.env.load_model_path
        if self.use_wandb:
        ...
        if self.model_dir is not None:
            self.restorer(self.env.load_model_name)
    ...
    def batch_train(self):
        ...
        print("self.policy_ids:", self.policy_ids)
    ...
    def save(self, cost=None, episode=None):
        ...
        for pid in self.policy_ids:
            ...
            if cost is None:
                ...
                torch.save(policy_critic.state_dict(),
                           critic_save_path + '/' + 'e_' + str(episode) + '_c_' + str(
                               np.around(cost, 5)) + '_' + 'critic.pt')
            ...
            if cost is None:
                ...
                torch.save(policy_actor.state_dict(),
                           actor_save_path + '/' + 'e_' + str(episode) + '_c_' + str(
                               np.around(cost, 5)) + '_' + 'actor.pt')

    def save_q(self, cost=None, episode=None):
    ...
    def restore(self, model_name=None):
        ...
        for pid in self.policy_ids:
            ...
            if model_name is None:
                ...
                policy_critic_state_dict = torch.load(path + '/'+model_name+'_critic.pt',
                                                      map_location=torch.device('cpu'))
                policy_actor_state_dict = torch.load(path + '/'+model_name+'_actor.pt',
                                                     map_location=torch.device('cpu'))
    ...
    def restore_q(self, model_name=None):
