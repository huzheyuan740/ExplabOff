...
class GR_Actor(nn.Module):
    ...
    def __init__(self, 
        ...
        obs_shape = obs_space[0]
        node_obs_shape = node_obs_space[1]
        edge_dim = edge_obs_space[0]

        self.gnn_base = GNNBase(args, node_obs_shape, edge_dim, args.actor_graph_aggr)
        gnn_out_dim = self.gnn_base.out_dim # output shape from gnns  TODO ?????
        mlp_base_in_dim = gnn_out_dim + obs_shape
    ...
    def forward(self,
        ...
        if (self.split_batch) and (obs.shape[0] > self.max_batch_size):
            ...
            # node_obs: torch.Size([384, 9, 7]) 7一般不变
            # adj: torch.Size([384, 9, 9])
            # agent_id: torch.Size([384, 1])  1一般不变
            nbd_features = self.gnn_base(node_obs, adj, agent_id)  # 这里应该对应论文图1的3.2节部分
            # nbd_features: torch.Size([384, 16])  16一般不变
            actor_features = torch.cat([obs, nbd_features], dim=1)
            # obs: torch.Size([384, 6]) 6一般不变
            # actor_features: torch.Size([384, 22])  22一般不变
            actor_features = self.base(actor_features)

        if self._use_naive_recurrent_policy or self._use_recurrent_policy:
            actor_features, rnn_states = self.rnn(actor_features, rnn_states, masks)
            # actor_features2: torch.Size([384, 64])
            # print("actor_features2:", actor_features.shape)

        actions, action_log_probs = self.act(actor_features,
                                            available_actions, 
                                            deterministic)
        # actions: torch.Size([384, 1])
...
class GR_Critic(nn.Module):
    ...
    def __init__(self, 
        ...
        cent_obs_shape = cent_obs_space[0]
        node_obs_shape = node_obs_space[1]
        edge_dim = edge_obs_space[0]
    ...
    def forward(self, 
        ...
        if (self.split_batch) and (cent_obs.shape[0] > self.max_batch_size):
            ...
            for batch in batchGenerator:
                obs_batch, node_obs_batch, adj_batch, agent_id_batch = batch
                nbd_feats_batch = self.gnn_base(node_obs_batch, adj_batch,   # 这里应该对应论文图1的3.3节
                                                agent_id_batch)
                act_feats_batch = torch.cat([obs_batch, nbd_feats_batch], dim=1)
