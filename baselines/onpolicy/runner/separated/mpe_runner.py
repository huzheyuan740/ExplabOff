...
from baselines.onpolicy.utils.util import update_linear_schedule
from baselines.onpolicy.runner.separated.base_runner import Runner
import imageio
...
class MPERunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(int(self.mec_config.train_config.episode_num)):
            env = self.envs
            ...
            for step in range(self.episode_length):
                ...
                for i in range(len(env.base_station_set.all_mobile_device_list)):
                    ...
                    meaningful_scaled_a[0] = int(
                        np.floor((scaled_a[0] + 1) * env.base_station_set.base_station_num / 2))
                    meaningful_scaled_a[1] = (scaled_a[1] + 1) / 2
                    print("meaningful_scaled_a:", meaningful_scaled_a)
                    action_class = Action(meaningful_scaled_a, self.mec_config)
                    scaled_a_list.append(scaled_a)
                    action_class_list.append(action_class)

                # print("self.buffer.obs[step]:", self.buffer.obs[step][0])

                # Obser reward and next obs
                next_state_class_list, cost_array, reward_array, cost_array_max, done_list, _ = env.step(
                    state_class_list,
                    action_class_list,
                ...
                terminal = False
                terminal = (step + 1 >= self.mec_config.train_config.step_num)
            ...
            env.writer.add_scalar("data/0episode_reward", episode_reward, episode)
            env.writer.add_scalar("data/0episode_cost", episode_cost_list[-1], episode)
            env.writer.add_scalar("data/0episode_cost_max", episode_cost_max_list[-1], episode)
            env.writer.add_scalar("data/0timesteps_in_episode", episode_timesteps, episode)
            env.writer.add_scalar("data/0success_rate_episode",
                                  (episode_timesteps - 1) / (self.mec_config.train_config.step_num - 1),
                                  episode)
    ...
    def warmup(self):
        # reset env
        self.envs.reset()
        self.envs.create_task_per_step()
