...
class MA_T_DDPG(object):
    ...
    def train_actor_with_mine(self, replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.001, max_mi_c=0.0,
        ...
        for it in range(iterations):
            ...
            if min_adv_c > 0.0 :
                ...
                MI_upper_bound = 0.0*torch.zeros_like(reward)
            if max_adv_c > 0.0 :
                with torch.no_grad():
                ...
                MI_lower_bound = 0.0*torch.zeros_like(reward)
    ...
    def save(self, filename, directory):
        ...
        torch.save(self.club_policy.state_dict(), '%s/%s_club_policy.pth' % (directory, filename))
    ...
    def load(self, filename, directory):
        ...
        self.club_policy.load_state_dict(torch.load('%s/%s_club_policy.pth' % (directory, filename)))
