...
def make_env(scenario_name, benchmark=False, discrete_action=False):
    ...
    if scenario_name == "predator_prey":
        print("~~~wildlife~~~:", benchmark)
        from multiagent.environment_wildlife import MultiAgentEnv
...
if __name__ == "__main__":
    ...
    while total_timesteps < train_config.total_timesteps:
        if episode_timesteps == train_config.step_num or done:  # episode_timesteps是指每个episode中的step_count的值 不会超过10应该
            ...
            if total_timesteps != 0:
                ...
                if len(avg_reward_list_for_best) > 10000:
                    if np.mean(avg_reward_list_for_best) > best_reward_for_save_model and np.std(avg_reward_list_for_best) < 100 and np.mean(avg_reward_list_for_best) > 500:
                        best_reward_for_save_model = np.mean(avg_reward_list_for_best)
                        policy.save("best_"+str(total_timesteps)+"cost_"+str(np.around(np.mean(avg_cost_list_for_best), 5))+"reward_"+str(np.around(np.mean(avg_reward_list_for_best), 3)), "model/" + file_name)
                    avg_reward_list_for_best = []
                    avg_cost_list_for_best = []
            ...
            if total_timesteps >= 1024 and total_timesteps % 100 == 0:
                ...
                for i in range(1):
                    ...
                    if len(good_data_buffer.pos_storage) < 500:
                        ...
                        print("MADDPG!")
                        exit()
                        if total_timesteps % (args.MI_update_freq * 100) == 0:  # 注100个timesteps才更新一次网络
                            update_signal = True
                        ...
                        process_Q, process_min_MI, process_max_MI, Q_grads, MI_grads, MI_upper_bound_list, MI_lower_bound_list, training_reward_list, training_reward_Q_list, training_r_PMIC_list = policy.train_actor_with_mine(
                            ...
                            min_adv_c=args.min_adv_c, max_adv_c=args.max_adv_c, total_timesteps=total_timesteps, update_signal=update_signal)
        ...
        # print("scaled_a_list:", scaled_a_list)

        # env.render()
        # print("env:", env)
        # a, scaled_a, obs[i] 是标准化或者神经网络输出值
        # state_class_list, action_class_list 都是有物理含义的值
        next_state_class_list, cost_array, reward_array, cost_array_max, done_list, _ = env.step(state_class_list, action_class_list,
                                                                                 episode_timesteps)
        # print("next_state_class_list:", next_state_class_list)
